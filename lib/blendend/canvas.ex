defmodule Blendend.Canvas do
  ...
  **user transform** in-place. 
  ...
  @doc """
  Sets the stroke width used by subsequent stroke operations.

  The width is a floating-point value in user–space units.
  """
  @spec set_stroke_width(t(), number()) :: :ok | {:error, term()}
  def set_stroke_width(canvas, width),
    do: Native.canvas_set_stroke_width(canvas, width * 1.0)

  @doc """
  Same as `set_stroke_width/2`, but returns the canvas.

  On success, returns `canvas`.

  On failure, raises `Blendend.Error`.
  """
  @spec set_stroke_width!(t(), number()) :: t()
  def set_stroke_width!(canvas, width) do
    case set_stroke_width(canvas, width) do
      :ok -> canvas
      {:error, reason} -> raise Error.new(:canvas_set_stroke_width, reason)
    end
  end
  ...
  `style` can be one of:
    ...
    * `Blendend.Style.Color.rgb!/4`      – solid color
    * `Blendend.Style.Gradient.t()` – gradient
    * `Blendend.Style.Pattern.t()`  – pattern
  """
  @spec set_stroke_style(t(), term()) :: :ok | {:error, term()}
  def set_stroke_style(canvas, style), do: Native.canvas_set_stroke_style(canvas, style)

  @doc """
  Same as `set_stroke_style/2`, but returns the canvas.

  On success, returns `canvas`.

  On failure, raises `Blendend.Error`.
  """
  @spec set_stroke_style!(t(), term()) :: t()
  def set_stroke_style!(canvas, style) do
    case set_stroke_style(canvas, style) do
      :ok -> canvas
      {:error, reason} -> raise Error.new(:canvas_set_stroke_style, reason)
    end
  end
  ...
  @doc """
  Sets the compositing operator used for subsequent drawing.

  The operator controls how new pixels are blended with the existing canvas
  contents. Common values:
    ...
    * `:exclusion`
  """
  @spec set_comp_op(t(), atom()) :: :ok | {:error, term()}
  def set_comp_op(canvas, op),
    do: Native.canvas_set_comp_op(canvas, op)

  @doc """
  Same as `set_comp_op/2`, but returns the canvas.

  On success, returns `canvas`.

  On failure, raises `Blendend.Error`.
  """
  @spec set_comp_op!(t(), atom()) :: t()
  def set_comp_op!(canvas, op) do
