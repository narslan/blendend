...
static int load(ErlNifEnv* env, void**, ERL_NIF_TERM)
{
  ...
  if(NifResource<Path>::open(env, "Elixir.Blendend.Native") < 0)
    return -1;
  if(NifResource<Matrix2D>::open(env, "Elixir.Blendend.Native") < 0)
    return -1;
  if(NifResource<Color>::open(env, "Elixir.Blendend.Native") < 0)
    return -1;
  if(NifResource<Gradient>::open(env, "Elixir.Blendend.Native") < 0)
    return -1;
  if(NifResource<Pattern>::open(env, "Elixir.Blendend.Native") < 0)
    return -1;
  if(NifResource<FontFace>::open(env, "Elixir.Blendend.Native") < 0)
    return -1;
  if(NifResource<Font>::open(env, "Elixir.Blendend.Native") < 0)
    return -1;
  if(NifResource<GlyphBuffer>::open(env, "Elixir.Blendend.Native") < 0)
    return -1;
  if(NifResource<GlyphRun>::open(env, "Elixir.Blendend.Native") < 0)
    return -1;
